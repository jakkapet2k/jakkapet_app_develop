...
public class LineGraph extends JPanel {
	...
	public LineGraph(int w, int h, int dataBound, int[] data) {
		...
		System.out.println("xAxisWidth:" + graphWidth + ", yAxisHeight:" + graphHeight + ", valueRatio:" + valueRatio);
	...
	public int[] scaleData(int[] raw) {
		...
		for (int data : raw) {
			...
			scaled[i++] = dataScaled;
			System.out.println("raw:" + data + ", temp:" + temp + ", dataScaled:" + dataScaled);
		}
	...
	public void paint(Graphics g) {
		...
		System.out.println(
				"yBlockSize:" + yBlockSize + ", xBlockSize:" + xBlockSize + ", dataBlockSize:" + dataBlockSize);
		...
		for (int i = 0; i < data.length - 1; i++) {
			...
			// draw graph between a pair of points
			g2.drawLine(x1, y1, x2, y2);
			x1 += xBlockSize;
